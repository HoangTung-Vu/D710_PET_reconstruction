...
def test_segment_order_is_stirs_storage_order(mini_info):
    _pd, info = mini_info
    assert stir_oracle.segment_order(info) == [0, 1, -1, 2, -2]


def test_derived_ring_pairs_match_stirs_own_count(mini_info):
    _pd, info = mini_info
    pairs = stir_oracle.plane_ring_pairs(info, 6)
    assert len(pairs) == synth_hs.num_planes(6)
    stir_oracle.check_ring_pairs(info, pairs)
...
def test_span_2_doubles_the_odd_planes_of_segment_0(mini_info):
    _pd, info = mini_info
    m = stir_oracle.ring_pair_multiplicity(info)
...
def test_multiplicity_is_one_value_per_stored_plane(mini_info):
    _pd, info = mini_info
    assert stir_oracle.ring_pair_multiplicity(info).size == synth_hs.num_planes(6)
...
def test_open_projdata_keeps_the_data_alive(stir, mini_hs):
    pd, info = stir_oracle.open_projdata(mini_hs)
