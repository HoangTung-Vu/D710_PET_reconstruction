...
Bq/mL × B →  counts     B = real rebinned trues / Σ AF · P(GE's Bq/mL image)      = 6.94×10⁻⁵ counts per Bq/mL·mm
                        s = A · B = 0.319 counts per SUV·mm, per 2D LOR (beds 0.293–0.397)
```
...
| calibrated `s`, adult exam | 0.319 counts per SUV·mm (beds 0.293–0.397) |
...
pytest tests/test_deeppet.py -q                               # 22 tests, ~5 s
